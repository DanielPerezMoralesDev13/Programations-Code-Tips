...
3. **Uso de `filter`:**
   ...
   - *El resultado de `filter` es un generador, que se almacena en la variable `price_filter`.*

4. **Impresión del Generador:** *La impresión de `price_filter` muestra el tipo `<filter object at 0x...>`, indicando que es un generador.*

5. **Conversión a Lista:**
   - *Para obtener los precios filtrados como una lista de enteros, se convierte el generador `price_filter` a una lista utilizando `list()`.*
...
- **Generador de Precios Filtrados:** *`price_filter` es el generador resultante de aplicar `filter` a `prices`, utilizando una función lambda para seleccionar los precios mayores o iguales a 20.*
