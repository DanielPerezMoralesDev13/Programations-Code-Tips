...
3. **Uso de `sum`**:
   ...
   - *El resultado de `sum(expensive)` es la suma de los elementos en la lista, que se almacena en la variable `total_sum`.*
...
- **Cálculo de la Suma Total:** *`total_sum` es la suma de todos los elementos en la lista `expensive`, calculada utilizando la función `sum`.*
